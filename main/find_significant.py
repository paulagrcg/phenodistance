...
nameandrankdist_hammingandprobs_pvals_d = defaultdict(dict)
...
with open('../data/to_analyse.pkl', 'rb') as f:
...
for file in files:
    ...
    if match:
        ...
        if p_value_upper < 0.05:
            nameandrankdist_hammingandprobs_pvals_d[tuple(names_to_analyse[seq_number])][(fRNAhammingdistance[tuple(names_to_analyse[seq_number])],fRNAprob1[tuple(names_to_analyse[seq_number])],fRNAprob2[tuple(names_to_analyse[seq_number])])] = p_value_upper
with open('../data/nameandrankdist_hammingandprobs_pvals_d.pkl', 'wb') as f:
    pickle.dump(nameandrankdist_hammingandprobs_pvals_d, f)